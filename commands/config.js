...
module.exports = {
  ...
  run: async (client, message, args, { GuildDB }) => {
    let Config = new MessageEmbed()
      .setAuthor("서버 설정", client.config.IconURL)
      .setColor("RANDOM")
...
`);
    ...
    let emoji = await ConfigMessage.awaitReactions(
      ...
      client.sendTime(
        message.channel, "❌ | **오랫동안 응답이 없어서 취소되었습니다.**"
      );
    ...
    if (em._emoji.name === "1️⃣") {
      await client.sendTime(message.channel, "봇의 접두사를 무엇으로 바꾸시겠습니까?");
      let prefix = await message.channel.awaitMessages(
        (msg) => msg.author.id === message.author.id,
        { max: 1, time: 30000, errors: ["time"] }
      );
      if (!prefix.first())
        return client.sendTime(message.channel, "오랫동안 응답이 없어서 취소되었습니다.");
      prefix = prefix.first();
      ...
      client.sendTime(
        message.channel, `서버의 봇 접두사를 성공적으로 변경했습니다 \`${prefix}\``
      );
    } else {
  ...
  SlashCommand: {
    options: [
      ...
      {
        name: "dj",
        description: "DJ role을 확인합니다",
        type: 1,
        required: false,
        options: [
          {
            name: "role",
            description: "DJ role을 세팅합니다",
            type: 8,
            required: false,
          },
        ],
    ...
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      if (config === "prefix") {
        ...
        if (
          ...
          client.sendTime(interaction, `서버의 봇 접두사를 성공적으로 변경했습니다 \`${prefix}\``);
        } else {
          //has not prefix
          client.sendTime(interaction, `서버의 봇 접두사 \`${GuildDB.prefix}\``);
        }
